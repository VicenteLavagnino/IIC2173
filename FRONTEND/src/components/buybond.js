...
const BuyBond = ({ fixtureId, homeTeam, awayTeam }) => {
  ...
  const handleBuyBondWallet = async () => {
    ...
    const paymentAmount = amountNumeric * 1000;
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_URL_API}/buy_bond`,
        {
  ...
  /* COMPRA BONOS ADMIN */
  const handleBuyBondAdmin = async () => {
    setError(null);
    setSuccess(null);
    const amountNumeric = parseFloat(amount);
    const paymentAmount = amountNumeric * 1000;
    ...
    try {
      ...
      console.log("Enviando solicitud de compra grupal...");
      const response = await axios.post(
        `${process.env.REACT_APP_URL_API}/bonds/group_buy`,
        {
          fixture_id: fixtureId.toString(),
          result: result,
          amount: amountNumeric,
  ...
  return (
    <div>
      ...
      <Modal isOpen={modal} toggle={toggle}>
        ...
        <ModalBody>
          ...
          <FormGroup>
            ...
            >
              ...
              <option value="home">Victoria Local ({homeTeam.name})</option>
              <option value="---">Empate</option>
              <option value="away">Victoria Visitante ({awayTeam.name})</option>
            </Input>
          </FormGroup>
          <FormGroup>
            <Label for="amount">Cantidad de bonos $1000 c/u</Label>
            <Input
              type="number"
              name="amount"
              id="amount"
        ...
        <ModalFooter>
          {admin && (
            <Button className="admin-buy-button" onClick={handleBuyBondAdmin}  disabled={loading}>
              {loading ? "Comprando..." : "Admin"}
            </Button>
          )}
