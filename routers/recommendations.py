...
async def create_recommendations(
    ...
    """
    task = process_recommendations.delay(
        current_user["sub"], request.min_recommendations
    )
...
async def get_user_recommendations(
    ...
    recommendations = (
        await db.recommendations.find({"user_id": current_user["sub"]})
        .sort("created_at", -1)
        .to_list(None)
    )
...
async def generate_recommendations_after_purchase(
    bond_id: str, current_user: dict = Depends(get_current_user)
):
    """
